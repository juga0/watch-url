...
def get_db_etag(etag_url):
    ...
    try:
        etag = r.json()['rows'][0]['value']['header']['etag']
        last_modified = r.json()['rows'][0]['value']['header']['last-modified']
...
def push_db_etag(url, etag, last_modified):
    ...
    logger.debug('post request to url %s returned status %s', url, r.status_code)
